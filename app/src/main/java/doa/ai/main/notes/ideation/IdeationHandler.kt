...
import doa.ai.main.notes.ideation.archived.ArchiveIdeationActivity
import doa.ai.main.notes.ideation.model.*
import doa.ai.main.notes.ideation.model.Result
import doa.ai.main.notes.ideation.trashed.TrashIdeationActivity
...
class IdeationFragment : BaseFragment(), IdeationView {



    companion object {
        fun newInstance(): IdeationFragment {
            return IdeationFragment()
        }
    }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        return inflater.inflate(doa.ai.R.layout.fragment_ideation, container, false)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        ideationPresenter.getDataIdeation("token $tokenLogin", 1,"published")
        setUpRecylerview()
        ...
        listNotes.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            ...
            override fun onScrollStateChanged(recyclerView: RecyclerView, newState: Int) {
                ...
                if (recyclerView.canScrollVertically(1).not()
                    ...
                    currentPage += 1
                    ideationPresenter.getDataIdeationAfter("token $tokenLogin", currentPage,"published")
...
//                DividerItemDecoration(context,   DividerItemDecoration.VERTICAL))
        ...
        override fun onActionItemClicked(p0: ActionMode?, item: MenuItem?): Boolean {
            when (item?.getItemId()) {
                ...
                doa.ai.R.id.action_add_trash -> {
                    getAllDataIdIdeation()
                    val body = BodyIdeaUpdateStatus(idIdeations,"trashed")
                    ideationPresenter.postUpdateStatusIdeation("token $tokenLogin", body)
                    Handler().postDelayed({
                        ideationPresenter.getDataIdeation("token $tokenLogin", currentPage,"published")
                    }, 1500)
    ...
    override fun onCreateOptionsMenu(menu: Menu, inflater: MenuInflater) {
        this.menu = menu
        inflater?.inflate(doa.ai.R.menu.menu_list_idea, menu)
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item?.itemId) {
            ...
            doa.ai.R.id.actions_archive_idea -> {
                intentTo(ArchiveIdeationActivity::class.java)
                true
            }
            doa.ai.R.id.actions_trash_idea -> {
                intentTo(TrashIdeationActivity::class.java)
