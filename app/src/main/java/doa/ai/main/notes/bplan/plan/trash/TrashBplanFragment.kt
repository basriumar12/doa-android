...
class TrashBplanFragment : BaseFragment(), BPlanView {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        tv_empty_data_bplan.text = "Kumpulan rencana dihapus akan muncul disini"
...
//        listBplanMain.smoothScrollToPosition(RecyclerView.VERTICAL)
        ...
        presenter.getlistBPlan("token $tokenLogin", 1, "", "trashed")
            ...
            override fun onScrollChange(v: NestedScrollView?, scrollX: Int, scrollY: Int, oldScrollX: Int, oldScrollY: Int) {
                ...
                if (v?.getChildAt(v?.getChildCount() - 1) != null) {
                    if ((scrollY >= (v?.getChildAt(v?.getChildCount() - 1).getMeasuredHeight() - v.getMeasuredHeight())) &&
                            scrollY > oldScrollY) {
                        currentPage += 1
                        presenter.getlistBPlanAfter("token $tokenLogin", currentPage, "", "trashed")
        ...
        override fun onActionItemClicked(p0: ActionMode?, item: MenuItem?): Boolean {
            when (item?.getItemId()) {
                ...
                doa.ai.R.id.action_add_trash -> {
                    ...
                    builder?.setTitle("Hapus untuk selamanya ?")


                    builder?.setPositiveButton("Hapus") { dialog, which ->
                        // Do something when user press the positive button
                        val bodyPinnedPlan = BodyDelPlan(idPlans)
                        presenter.deletePlan("token $tokenLogin", bodyPinnedPlan)
                        Handler().postDelayed({
                            presenter.getlistBPlan("token $tokenLogin", 1, "", "trashed")
...
//                    val bodyCopyPlan = BodyCopyPlan(idPlans)
        ...
        override fun onCreateActionMode(p0: ActionMode?, p1: Menu?): Boolean {
            p0?.menuInflater?.inflate(doa.ai.R.menu.menu_trash_bplan, p1)
...
//            adapterBplanMain.notifyDataSetChanged()
    ...
    override fun successEditBPLan(planResponse: PlanResponse) {
        ...
        showInfoMessage(planResponse.message.toString())
        presenter.getlistBPlan("token $tokenLogin", 1, "", "trashed")
