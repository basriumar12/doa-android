...
class AdapterCustomerManagements (private val item: ArrayList<String>, private val contex: Context)
    : RecyclerView.Adapter<AdapterCustomerManagements.ViewHolder>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int):
            ViewHolder = ViewHolder(LayoutInflater.from(parent.context)
            .inflate(R.layout.view_customer_managements, parent, false))
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
       holder.itemView.img_close.setOnClickListener {
        ...
        holder.itemView.edt_customer_managements.setText(item.get(position).toString())
        holder.itemView.edt_customer_managements.addTextChangedListener(object : TextWatcher{
            override fun afterTextChanged(p0: Editable?) {
           SavePrefSegmentMarket().setMarketCustomerManagement(p0.toString())
            }
            ...
            override fun onTextChanged(p0: CharSequence?, p1: Int, p2: Int, p3: Int) {
                    item[position] = holder.itemView.edt_customer_managements.text.toString()
