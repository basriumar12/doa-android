...
import doa.ai.main.notes.bplan.plan.archived.ArchivedBplanActivity
import doa.ai.main.notes.bplan.plan.search.SearchBPlanActivity
import doa.ai.main.notes.bplan.plan.trash.TrashBplanActivity
import doa.ai.profile.collab.CollaborationActivity


class BPlanFragment : BaseFragment(), BPlanView {



    companion object {
        fun newInstance(): BPlanFragment {
            return BPlanFragment()
        }
    ...
    val idPlansPinned : MutableList<Int> = mutableListOf()
    val idPlansPinnedForDelete : MutableList<Int> = mutableListOf()
    var idPlan = 0
...
//        listBplanMain.smoothScrollToPosition(RecyclerView.VERTICAL)
        ...
        presenter.getlistBPlan("token $tokenLogin", 1, "", "completed_or_drafted")
            ...
            override fun onScrollChange(v: NestedScrollView?, scrollX: Int, scrollY: Int, oldScrollX: Int, oldScrollY: Int) {
                ...
                if(v?.getChildAt(v?.getChildCount() - 1) != null) {
                    if ((scrollY >= (v?.getChildAt(v?.getChildCount() - 1).getMeasuredHeight() - v.getMeasuredHeight())) &&
                            scrollY > oldScrollY) {
                        currentPage += 1
                        presenter.getlistBPlanAfter("token $tokenLogin", currentPage, "", "completed_or_drafted")
...
//                        presenter.getlistBPlanPinnedAfter("token $tokenLogin", currentPagePinned, "")
        ...
        listBplanPinned.addOnItemTouchListener(RecyclerTouchListener(activity, listBplanPinned, object : RecyclerClick_Listener{
            override fun onLongClick(view: View?, position: Int) {
        ...
        adapter.setOnItemClickListener(object : AdapterBPlan.OnItemClickListener {
...
//                    .selectedCount.toString() + " dipilih"
    ...
    override fun onSuccessBPlan(response: MutableList<Result>) {
...
//        toast.show()
    ...
    override fun onCreateOptionsMenu(menu: Menu?, inflater: MenuInflater?) {
        ...
        inflater?.inflate(doa.ai.R.menu.menu_list, menu)
    ...
    override fun onOptionsItemSelected(item: MenuItem?): Boolean {
        return when (item?.itemId) {
            ...
            doa.ai.R.id.actions_archive -> {
                context?.startActivity(Intent(context, ArchivedBplanActivity::class.java)
                        )
                true
            }
            doa.ai.R.id.actions_trash -> {
                context?.startActivity(Intent(context, TrashBplanActivity::class.java)
                        )
                true
            }
    ...
    private fun getAllDataIdPlans() {
        ...
        idPlan = listResult.get(selected.keyAt(0)).id
        idPlans.clear()
    ...
    private fun getAllDataIdPlansPinned() {
        val selected: SparseBooleanArray
        selected = adapterPinned.selectedIds!!
        idPlan = listResultPinned.get(selected.keyAt(0)).id
        idPlansPinned.clear()
        for (data in 0 until selected.size() ) {
            if (selected.valueAt(data)) {
                val dataSelect = listResultPinned.get(selected.keyAt(data)).id
                Log.e("TAG", "data select $dataSelect")
                idPlansPinned.add( listResultPinned.get(selected.keyAt(data)).id)
            }

        }
    }

    private fun getAllDataIdPlansPinnedDelete() {
        val selected: SparseBooleanArray
        selected = adapterPinned.selectedIds!!
        idPlansPinnedForDelete.clear()
        for (data in 0 until selected.size() ) {
            if (selected.valueAt(data)) {
                val dataSelect = listPinned.get(selected.keyAt(data)).id
                Log.e("TAG", "data select $dataSelect")
                idPlansPinnedForDelete.add( listPinned.get(selected.keyAt(data)).id)
            }
...
//                DividerItemDecoration(context,   DividerItemDecoration.VERTICAL))
    ...
    private val mActionModeCallbackUnPinned = object : ActionMode.Callback {
        override fun onActionItemClicked(p0: ActionMode?, item: MenuItem?): Boolean {
            when (item?.getItemId()) {
                doa.ai.R.id.action_add_pinned -> {
                    getAllDataIdPlansPinnedDelete()
                    val bodyPinnedPlan = BodyDelPinnedPlans(idPlansPinnedForDelete)
                    presenter.deletePinnedPlan("token $tokenLogin", bodyPinnedPlan)
                    Handler().postDelayed({
                        presenter.getlistBPlan("token $tokenLogin", 1, "", "completed_or_drafted")
                        presenter.getlistBPlanPinned("token $tokenLogin", 1, "")
                ...
                doa.ai.R.id.action_add_trash -> {
                    ...
                    getAllDataIdPlansPinned()

                    val body = BodyEditPlans(idPlansPinned,"trashed")
                    presenter.sendArchivedDataPlans("token $tokenLogin", body)
                    Handler().postDelayed({
                        presenter.getlistBPlan("token $tokenLogin", 1, "", "completed_or_drafted")
                        presenter.getlistBPlanPinned("token $tokenLogin", 1, "")
                ...
                doa.ai.R.id.action_add_archived -> {
//                    val partners: MutableList<Int> = mutableListOf()
//                    val bodyEditBplan = BodyEditBplan(
//                            resultPinned.plan.end_date,
//                            resultPinned.plan.reminder_time,
//                            resultPinned.plan.reminder,
//                            partners,
//                            resultPinned.plan.sub_sector.id,
//                            resultPinned.plan.repeat,
//                            resultPinned.plan.scale,
//                            resultPinned.id,
//                            resultPinned.plan.title,
//                            resultPinned.plan.start_date,
//                            "archived"
//                    )
...
//                    presenter.sendArchivedDataPlan("token $tokenLogin", bodyEditBplan)
                    ...
                    getAllDataIdPlansPinned()
                    val body = BodyEditPlans(idPlansPinned,"archived")
                    presenter.sendArchivedDataPlans("token $tokenLogin", body)
                    Handler().postDelayed({
                        presenter.getlistBPlan("token $tokenLogin", 1, "", "completed_or_drafted")
                        presenter.getlistBPlanPinned("token $tokenLogin", 1, "")
                ...
                doa.ai.R.id.action_add_copy -> {
                    ...
                    getAllDataIdPlansPinned()
                    val bodyCopyPlan = BodyCopyPlan(idPlansPinned)
                    presenter.sendCopyPlan("token $tokenLogin", bodyCopyPlan)
                    Handler().postDelayed({
                        presenter.getlistBPlan("token $tokenLogin", 1, "", "completed_or_drafted")
                        presenter.getlistBPlanPinned("token $tokenLogin", 1, "")
