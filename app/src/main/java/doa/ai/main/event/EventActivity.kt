...
class EventActivity : BaseActivity(), EventView {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        checkRegister = getIntent().getExtras().getBoolean("checkRegister")
        withBplan = getIntent().getExtras().getBoolean("bplan")
        withDocument = getIntent().getExtras().getBoolean("document")
        flag = intent.getStringExtra("flag")
        Log.e("tag", "$withDocument  $withBplan $checkRegister")
        Picasso.get().load(url).fit().into(imageEvent)
        ...
        presenter.getCheckRegisterEvent("token $tokenLogin", id!!)
    ...
    fun uploadFileReady() {
        ...
        service.postUploadFile("Token $nameImageSha256", convertFolderName, fileUpload).enqueue(object : retrofit2.Callback<ResponseUploadImage> {
            ...
            override fun onResponse(call: Call<ResponseUploadImage>, response: Response<ResponseUploadImage>) {
                loading.hide()
                Log.e("TAG upload","${Gson().toJson(response.body())}")
